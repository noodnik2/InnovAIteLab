...
async function doit() {
    ...
    const queries = [
        "What year were movies directed by Greta Gerwig?",
    ...
    const chain = RetrievalQAChain.fromLLM(llm, selfQueryRetriever);

    console.log(`temperature: ${llm.temperature}`)
    for (const query of queries) {
        const relevantDocs = await selfQueryRetriever.getRelevantDocuments(query);
        console.log(`documents found for query(${query}):\n=> ${JSON.stringify(relevantDocs)}`);
